...
namespace prjProduct_core.Controllers
{
    public class Admin_ApiController : Controller
    {
        ...
        public Admin_ApiController(CoffeeContext context, IWebHostEnvironment environment, IConfiguration configuration)
        {
            _context = context;
        ...
        public IActionResult SendMailToken(CForgotPasswordViewModel inModel)
        {
            ...
            if (_context.Admins.Any(a => a.Email == inModel.Email))
            {
                ...
                DES.Key = Encoding.UTF8.GetBytes(md5Key);
                DES.Mode = CipherMode.ECB;
                ICryptoTransform DESEncrypt = DES.CreateEncryptor();
                byte[] Buffer = Encoding.UTF8.GetBytes(sVerify);
                ...
                // 從信件連結回到重設密碼頁面
                string receivePage = "Admin_Dashboard/ResetPassword";
        ...
        public ActionResult DoResetPwd(CDoResetPwdIn inModel)
        {
            ...
            string userEmail = HttpContext.Session.GetString(CDictionary.SK_ResetPassword_AdminId);
