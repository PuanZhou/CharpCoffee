...
{
    public class Admin_MemberController : Controller
    {
            ...
            if (HttpContext.Session.Keys.Contains(CDictionary.SK_LOGINED_ADMIN))
            {
                ...
                if (signIn_User.MemberOk)
                {
                    ...
                    if (string.IsNullOrEmpty(vModel.txtKeyword)) // 若沒輸入關鍵字則回傳所有會員
                    {
                        ...
                        {
                            CAdmin_MemberViewModel MemVModel = new CAdmin_MemberViewModel();
                            MemVModel.member = m;
                            list.Add(MemVModel);
                        }
                    }
        ...
        public IActionResult Details(int? id)
        {
            if (HttpContext.Session.Keys.Contains(CDictionary.SK_LOGINED_ADMIN))
            {
                ...
                if (signIn_User.MemberOk)
                {
                    ...
                    var list = _context.Orders.Where(o => o.MemberId == id).Select(o => new CAdmin_OrderViewModel()
                    {
                        order = o,
                        Payment = o.Payment,
                        Coupon = o.Coupon,
        ...
        public IActionResult Filter1(int? id)
        {
            ...
            var dborder = _context.Orders.Where(o => o.MemberId == currentMembersOrderDetails).Select(t => new CAdmin_OrderViewModel
            {
                OrderId = t.OrderId,
                Payment = t.Payment,
